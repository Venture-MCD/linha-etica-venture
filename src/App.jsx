...
const Field = ({ label, required, hint, children }) => (
  <label className="grid gap-1" style={{ gridTemplateRows: "minmax(40px,auto) minmax(18px,auto) auto" }}>
    <span className="text-sm font-medium leading-5">
      {label} {required && <span className="text-rose-600">*</span>}
    </span>
    <div className={`text-xs leading-4 ${hint ? "text-slate-500" : "opacity-0"}`}>{hint || "\u00A0"}</div>
    {children}
  </label>
);
  ...
  <div className="relative">
    <select
      ...
      className={
        ...
        "focus:outline-none focus:ring-2 focus:ring-emerald-600 " + className
      }
    >{children}</select>
    <svg aria-hidden="true" className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 opacity-70" viewBox="0 0 20 20" fill="currentColor">
...
const Card = ({ className, children }) => <div className={`rounded-xl border p-6 bg-white shadow ${className || ""}`}>{children}</div>;
...
const Nav = () => (
  <nav className="flex items-center justify-between mb-6">
    <a href="#/" className="flex items-center gap-2">
      <img src={logo} alt="Venture" className="h-7 w-auto object-contain" />
...
/* =========================================================
   HOME (igual à sua) + cabeçalho curto com logo
   ========================================================= */
function Home() {
  return (
    <section id="home" className="space-y-6">
      <div className="flex items-center gap-2">
        <img src={logo} alt="Venture" className="h-8 w-auto object-contain" />
